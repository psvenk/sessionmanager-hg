...
with (com.morac) {
	...
	com.morac.gSessionSaverConverter = {
		...
		init: function() {
			...
			try {
				if (gSessionManager.mPref_encrypt_sessions) SECRET_DECODER_RING_SERVICE.encryptString("");
			}
			catch(ex) {
			...
			if (aObj.value || aObj2.value) {
				...
				if (!this.confirm(gSessionManager._string("ss_confirm_import"))) this.importSession();
			}
			this.prefService = null;
			this.rootBranch = null;
			this.Branch = null;
